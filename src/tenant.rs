...
//!
use crate::client::{ManagementClient, ObjectstoreClient, AUTH_HEADER_KEY};
...
impl Namespace {
    pub(crate) fn create(
        ...
        let resp: Namespace = serde_json::from_str(&text).with_context(|| {
            format!(
                "Unable to deserialise CreateNamespace. Body was: \"{}\"",
                text
            )
        })?;
        Ok(resp)
    ...
    pub(crate) fn get(client: &mut ManagementClient, id: &str) -> Result<Namespace> {
        ...
        let text = get_content_text(resp)?;
        let resp: Namespace = serde_json::from_str(&text).with_context(|| {
            format!("Unable to deserialise GetNamespace. Body was: \"{}\"", text)
        })?;
        Ok(resp)
    }
